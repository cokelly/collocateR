...
pmi_score <- function(vect, keyword, window = 5, ngram = 1:4, cutoff = 3, parallel = TRUE){
      ...
      if(isTRUE(parallel) == TRUE){
            vect_phrases <- parallelizeTask(makeTokens, vect, ngram)
      } else {
      vect_phrases <- tokenize(vect, n = ngram, concatenator = " ", removeSymbols = TRUE, removeNumbers = TRUE, simplify = TRUE)
      }
      # Create a data frame with ngram frequency for each phrase
      vectdf <- as.data.table(table(unlist(vect_phrases)), row.names = NULL, stringsAsFactors = FALSE) 
      # A column with the probability for each word
      colnames(vectdf) <- c("phrase", "doc_freq")
      word_probs <- vectdf$doc_freq/sum(vectdf$doc_freq)
      ...
      if(isTRUE(parallel) == TRUE){
            kwic_phrases <- parallelizeTask(makeTokens, input = kwic_corp, n = ngram)
      } else {
            vect_phrases <- tokenize(vect, n = ngram, concatenator = " ", removeSymbols = TRUE, removeNumbers = TRUE, simplify = TRUE)}
      kwicdf <- as.data.table(table(unlist(kwic_phrases)), row.names = NULL, stringsAsFactors = FALSE)
      colnames(kwicdf) <- c("phrase", "kwic_freq")
      kwic_probs <- kwicdf$kwic_freq/sum(kwicdf$kwic_freq)
      kwicdf <- cbind(kwicdf, kwic_probs)
      colnames(kwicdf) <- c("phrase", "kwic_freq", "kwic_prob")
      setkey(kwicdf, phrase)
      # Merge both dfs
      overalldf <- vectdf[.(kwicdf)]
      ...
      overalldf <- overalldf2[order(overalldf2$pmi, decreasing = TRUE),]
      return(overalldf2)
}