...
#' @importFrom quanteda corpus kwic is.corpus tokenize
...
pmi_score <- function(vect, keyword, window = 5, ngram = 1:4, cutoff = 3){
      ...
      kwic_corp <- kwic_to_corpus(vect, keywords = keyword, window = window)
      ...
      mim <- log2(probxy/probxproby)
      # Add to df
      overalldf <- data.frame(cbind(overalldf$phrase, overalldf$doc_freq, overalldf$kwic_freq, mim))
