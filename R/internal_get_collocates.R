...
get_collocates <- function(document, pattern, window = window, ngram = ngram, remove_stopwords = remove_stopwords, remove_numerals = remove_numerals, cache = cache){
  # Ensure there are no spaces in the pattern
  if(length(ngram) == 1 && ngram == 1){
    doc_locations <- get_unigrams(document = document, pattern = pattern, window = window, remove_stopwords = remove_stopwords, remove_numerals = remove_numerals, cache = cache)
    doc.t2 <- doc_locations[[1]]
    pattern_locations <- doc_locations[[2]]
  }
  
  if(length(ngram) == 1 && ngram > 1){
    doc_locations <- get_multigrams(document = document, pattern = pattern, ngram = ngram, window = window, remove_stopwords = remove_stopwords, remove_numerals = remove_numerals, cache = cache) 
    doc.t2 <- doc_locations[[1]]
    pattern_locations <- doc_locations[[2]]
  }
